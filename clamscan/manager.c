...
extern int scanmem(struct cl_engine *trie, const struct optstruct *opts, const struct cl_scan_options *options);
...
static void scanfile(const char *filename, struct cl_engine *engine, const struct optstruct *opts, struct cl_scan_options *options)
{
    ...
    if (ret == CL_VIRUS && action) {
        ...
        if (optget(opts, "keep-mbox")->enabled) {
            fmap_t *maptype = fmap(fd, 0, 0, filename);
            if (maptype) {
                type = cli_determine_fmap_type(maptype, engine, CL_TYPE_BINARY_DATA);
                funmap(maptype);
            }
        }
        close(fd);
